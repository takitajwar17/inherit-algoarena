...
export const submitQuestAttempt = async (attemptId, answers) => {
  try {
    ...
    // Get the first answer and its corresponding question
    const firstAnswer = answers[0];
    const question = attempt.questId.questions.find(
      q => q._id.toString() === firstAnswer.questionId
    );
    ...
    const evaluation = await evaluateQuestAnswer(attempt.questId, firstAnswer.answer, question);
